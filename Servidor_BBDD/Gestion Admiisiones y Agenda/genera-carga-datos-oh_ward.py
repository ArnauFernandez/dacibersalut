...
try:
    ...
    for _ in range(num_records):
        ...
        wrd_name = fake.company()[:20]  # Nombre aleatorio con máximo 20 caracteres
        wrd_tele = fake.phone_number()[:15]  # Teléfono aleatorio
        wrd_fax = fake.phone_number()[:15]  # Fax aleatorio
        wrd_email = fake.company_email()[:50]  # Correo aleatorio
        wrd_nbeds = random.randint(5, 50)  # Número aleatorio de camas
        wrd_nqua_nurs = random.randint(1, 10)  # Número de enfermeras
        wrd_ndoc = random.randint(1, 5)  # Número de doctores
        wrd_is_opd = random.choice([0, 1])  # ¿Tiene OPD?
        wrd_is_pharmacy = random.choice([0, 1])  # ¿Tiene farmacia?
        wrd_is_male = random.choice([0, 1])  # ¿Es masculino?
        wrd_is_female = random.choice([0, 1])  # ¿Es femenino?
        wrd_visit_duration = random.randint(15, 60)  # Duración de visita en minutos
        wrd_lock = 0
    ...
    connection.commit()
    print(f"{num_records} registros falsos insertados en oh_ward.")
